...
class Flag extends Base
{
	...
	public function __construct(&$args, Factory $view, Repository $config)
	{
		...
		if (isset($args[0])) {
			if(isset($this->avlCodes[strtoupper($args[0])])) {
				$this->widgetArgs['code'] = strtolower($args[0]);
				$this->widgetArgs['countryName'] = $this->avlCodes[strtoupper($args[0])];
			} else throw new ThemeException('Country code is incorrect');
		}
		else throw new ThemeException('First argument (Code) is required for '.self::WIDGET_NAME.' element');
	}
